...
> validate it with `pnpm verify:local`, render headless with
> `pnpm render:local`. Start from `docs/examples/text-only.json`.
> (`pnpm verify` is the canonical API-backed check.)
